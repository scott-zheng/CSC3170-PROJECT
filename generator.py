...
def generate_user(n, used_id):
    i = 0
    user = []
    while i < n:
        user_id = random.randint(10000000, 99999999)
        if user_id not in used_id:
            used_id.add(user_id)
            i += 1
            username = gen_two_words()
            phone_number = str(random.randint(10000000000, 99999999999))
            user_password = ''.join(random.sample(string.ascii_letters + string.digits,8)) 
            user.append((user_id, username, phone_number, user_password))
    
    return user
            
def generate_customer(n, used_id, customer_id, vendor_id, school_id, college_id):
    i = 0
    school = list(school_id)
    college = list(college_id)
    gender = ['male', 'female']
    customer = []
    for id in used_id:
        if (id not in customer_id) and (id not in vendor_id):
            i += 1
            customer_id.add(id)
            birthday = gen_year(1990, 2005) + '-' + gen_birthday()
            cus_school = random.choice(school)
            cus_college = random.choice(college)
...
def generate_vendor(n, used_id, customer_id, vendor_id):
    i = 0
    vendor = []
    for id in used_id:
        if (id not in customer_id) and (id not in vendor_id):
    ...
    for id in used_id:
        if (id not in customer_id) and (id not in vendor_id):
            i += 1
            vendor_id.add(id)
            vendor.append((id, 'LeTian #1 Cafeteria', 'Lower Campus - LeTian Bldg. 1F', '3122661240', 'Food Supply with Reasonable Price', '10:30:00', '21:00:00'))
    return vendor

def generate_school(school_id):
    school_name = ['SSE', 'SME', 'HSS', 'LHS']
    school = []
    for i in school_name:
        id = random.randint(10000000, 99999999)
        school_id.add(id)
        director = gen_two_words()
        contact = str(random.randint(10000000000, 20000000000))
        school.append((id, i, director, contact))
    return school

def generate_college(college_id):
    college_name = ['MUSE', 'DILLIGENT', 'SHAW', 'HARMONIA']
    college = []
    for i in college_name:
        ...
        director = gen_two_words()
        contact = str(random.randint(10000000000, 20000000000))
        college.append((id, i, director, contact))
    return college


def generate_pre_tag(n, tag_id, customer_id):
    pre_tag = []
    tags = list(tag_id)
    customers = list(customer_id)
    for i in range(n):
        tag = random.choice(tags)
        customer = random.choice(customers)
        if ((tag, customer) not in pre_tag):
            pre_tag.append((tag, customer))
    return pre_tag

def generate_tag(tag_id):
    tag_name = ['Comfortable', 'Cosy', 'Famaly', 'Fun', 'Young', 'Quite', 'Clean', 'Fancy', 'Interesting', 'Yummy', 'Delicious']
    tags = []
    for i in tag_name:
        id = random.randint(10000000, 99999999)
        tag_id.add(id)
...
def generate_category(category_id):
...
def generate_follow(n, customer_id):
    i = 0
    customers = list(customer_id)
    follows = []
    while i < n:
        following = random.choice(customers)
        follower = random.choice(customers)
        if following != follower and ((following, follower) not in follows):
            i += 1
            follows.append((following, follower))
    return follows
...
def gen_datetime():
    ...
    datetime = time.strftime(("%Y-%m-%d %H:%M:%S"), date_touple)  
    return datetime


def generate_comment(n, customer_id, vendor_id):
    possible_comments = ["值得一去", "非常不错，印象深刻", "挺不错的，就是有点贵", "一般般", "服务态度很好", "强烈推荐！！！", "一言难尽。。。", ":)"]
    i = 0
    customers = list(customer_id)
    vendors = list(vendor_id)
    comments = []
    while i < n:
        cid = random.randint(10000000, 99999999)
        if cid not in comment_id:
            i += 1
            comment_id.add(cid)
            customer = random.choice(customers)
            vendor = random.choice(vendors)
            datetime = gen_datetime()
            content = random.choice(possible_comments)
            comments.append((cid, vendor, customer, content, datetime))
    return comments


def generate_attitude(n, comment_id, customer_id):
    i = 0
    comments = list(comment_id)
    customers = list(customer_id)
    attitudes = []
    while i < n:
        ...
        if (comment, customer, 1, 0) not in attitudes and (comment, customer, 0, 1) not in attitudes:
            i += 1
            attitudes.append((comment, customer, like, 1-like))
    return attitudes


def generate_photo(n, user_id):
    i = 0
    users = list(user_id)
    photos = []
    while i < n:
        pid = random.randint(10000000, 99999999)
        if pid not in photo_id:
            i += 1
            user = random.choice(users)
            datetime = gen_datetime()
            content = "./img/" + str(user) + ".jpg"
            photos.append((pid, content, datetime, user))
    return photos
...
n = 1000
user = generate_user(n, used_id)

school = generate_school(school_id)

college = generate_college(college_id)

customer = generate_customer(n, used_id, customer_id, vendor_id, school_id, college_id)

vendor = generate_vendor(n, used_id, customer_id, vendor_id)

category = generate_category(category_id)

tag = generate_tag(tag_id)

prefer_tag = generate_pre_tag(n, tag_id, customer_id)
...
# insert college
sql = "INSERT into College (College_id, College_name, Director, Contact) values (%s, %s, %s, %s)"
for i in college:
    cursor.execute(sql, i)
    db.commit()

# insert tag
sql = "INSERT into Tag (Tag_id, Tag_name) values (%s, %s)"
for i in tag:
    cursor.execute(sql, i)
    db.commit()

# insert user
sql = "INSERT into User (User_id, Person_NAME, Phone_number, Person_Password) values (%s, %s, %s, %s)"
for i in user:
    cursor.execute(sql, i)
    db.commit()

# insert customer
sql = "INSERT into Customer (User_id, Email, Birthday, School, college, Gender) values (%s, %s, %s, %s, %s, %s)"
for i in customer:
    cursor.execute(sql, i)
    db.commit()
...
# insert category
sql = "INSERT into Category (Category_id, Category_name, Category_desc) values (%s, %s, %s)"
for i in category:
    cursor.execute(sql, i)
    db.commit()

#insert perfer_tag
sql = "INSERT into Preference_tag (Tag_id, User_id) values (%s, %s)"
for i in prefer_tag:
    cursor.execute(sql, i)
    db.commit()

#insert follow
sql = "INSERT into Follow (Following_id, Follower_id) values (%s, %s)"
for i in follow:
    cursor.execute(sql, i)
    db.commit()

#insert comment
sql = "INSERT into Comment (Comment_id, Vendor_id, Customer_id, Content, `Time`) values (%s, %s, %s, %s, %s)"
for i in comment:
    cursor.execute(sql, i)
    db.commit()

#insert attitude
sql = "INSERT into Attitude_toward (Comment_id, Customer_id, `Like`, Dislike) values (%s, %s, %s, %s)"
for i in attitude:
    cursor.execute(sql, i)
    db.commit()

#insert photo
sql = "INSERT into Photo (Photo_id, Content, `Time`, User_id) values (%s, %s, %s, %s)"
for i in photo:
    cursor.execute(sql, i)
    db.commit()
